 ...
 * TODO: Student Implement
 *
 * @brief:Executes the commands in a file.
 *
 * This function reads commands from a file and executes them one by one. It parses each command using the Minisql parser,
 * executes the parsed command, and handles the result. The function continues executing commands until the end of the file
 * is reached or a quit command is encountered.
 *
 * @param ast The abstract syntax tree (AST) representing the command to execute.
 * @param context The execution context.
 * @return Returns DB_FAILED if the file cannot be opened, DB_QUIT if a quit command is encountered, or DB_SUCCESS otherwise.
 */
